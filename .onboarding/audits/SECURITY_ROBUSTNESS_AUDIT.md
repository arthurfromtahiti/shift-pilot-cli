...
**Fichier absent ou inaccessible.** `VÉRIFIÉ_CODE` : même chemin de code (`bin/index.js:9`), `fs.readFileSync` est appelé sans `try/catch` — toute erreur système est non capturée et remonte comme exception Node non gérée. `HYPOTHÈSE` : le cas typique lève `Error: ENOENT: no such file or directory` pour un fichier absent, ou `Error: EACCES: permission denied` pour un fichier inaccessible — comportement Node documenté, non reproduit dans cette analyse.
